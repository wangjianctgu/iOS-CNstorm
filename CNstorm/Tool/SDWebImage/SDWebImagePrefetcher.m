...
- (void)startPrefetchingAtIndex:(NSUInteger)index
{
    ...
    [self.manager downloadWithURL:self.prefetchURLs[index] options:self.options progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished)
    {
        ...
        if (image)
        {
            NSLog(@"Prefetched %d out of %d", (int)self.finishedCount, (int)self.prefetchURLs.count);
        }
        else
        {
            NSLog(@"Prefetched %d out of %d (Failed)", (int)self.finishedCount, (int)[self.prefetchURLs count]);
...
- (void)reportStatus
{
    ...
    NSLog(@"Finished prefetching (%d successful, %d skipped, timeElasped %.2f)", (int)(total - self.skippedCount), (int)self.skippedCount, CFAbsoluteTimeGetCurrent() - self.startedTime);
}
