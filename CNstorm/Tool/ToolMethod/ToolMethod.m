...
+ (BOOL)CheckPropForObject:(NSString*)propName forObject:(Class) class
{
   ...
   //objc_objectptr_t *popertylist = class_copyPropertyList(class,&nCount);//消除警告
    ...
    for (int i = 0; i < nCount; i++)
    {
        ...
        if([propName compare:[NSString stringWithUTF8String:propertyName]] == NSOrderedSame)
            return YES;
    }
    return NO;
}
...
-(id)initWithSubString:(NSString*) _substring before:(BOOL)_before keepsubstring:(BOOL)_keepsubstring  NSStringCompareOptions:(NSStringCompareOptions) _compareOptions
{
    if(self=[super init])
    {
        ...
        self.compareOptions = _compareOptions;
    }
