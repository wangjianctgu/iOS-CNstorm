...
-(void)initGoodInfoView
{
    ...
    //线条
    UIImageView *imageView1 = [[UIImageView alloc] initWithFrame: CGRectMake(275.0f ,10.0f, 1.0f, 30.0f)];
    [imageView1 setImage:[UIImage imageNamed:@"myCNstormline"]];
    [nameView addSubview: imageView1];
    
    self.favoriteButton = [[UIButton alloc]initWithFrame:CGRectMake(280.0f, 5.0f, 24.5f,23.5f)];
    ...
    [self.favoriteButton setBackgroundImage:[UIImage imageNamed:@"favorited"] forState:UIControlStateSelected];
    self.favoriteButton.selected = NO;
    [self.favoriteButton addTarget:self action:@selector(favorited:) forControlEvents:UIControlEventTouchUpInside];
    [nameView addSubview:self.favoriteButton];
    
    self.favoriteLabel = [[UILabel alloc]initWithFrame:CGRectMake(280.0f, 30.0f, 24.5f, 20.0f)];
    self.favoriteLabel.text = [NSString stringWithFormat:@"收藏"];
    self.favoriteLabel.textColor = [UIColor colorWithRed:(51.0f/255.0f) green:(51.0f/255.0f) blue:(51.0f/255.0f) alpha:1.0f];
    self.favoriteLabel.font = [UIFont systemFontOfSize:12.0f];
    self.favoriteLabel.textAlignment = NSTextAlignmentCenter;
    self.favoriteLabel.lineBreakMode = NSLineBreakByTruncatingTail;
    self.favoriteLabel.baselineAdjustment = UIBaselineAdjustmentNone;
    [self.favoriteLabel addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(favorited:)]];
    self.favoriteLabel.userInteractionEnabled = YES;
    [nameView addSubview:self.favoriteLabel];
    
    //查询该用户是否收藏该商品
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    if ([userDefaults boolForKey:@"isLogin"])
    {
        [self callSearchFavoriteWebServiceWithProductId:self.productId];
    }
...
- (void)favorited:(id)sender
{
    ...
    else
    {
        ...
        {
            self.favoriteLabel.textColor = [UIColor colorWithRed:(251.0f)/255.0f green:(110.0f)/255.0f blue:(83.0f)/255.0f alpha:1];
            
            [self callFavoriteWebServiceWithProductId:self.productId];
        }
        else
        {
            self.favoriteLabel.textColor = [UIColor colorWithRed:(51.0f/255.0f) green:(51.0f/255.0f) blue:(51.0f/255.0f) alpha:1.0f];
            
            [self callCancelFavoriteWebServiceWithProductId:self.productId];
        }
    }
}
...
-(void)commit:(id)sender
{
    ...
    {
        long long productId = self.productId;//本地商品id
        NSString *colorId = self.selectedColorId;
        NSString *sizeId = self.selectedSizeId;
        NSString *color = self.selectedColor;
        NSString *size = self.selectedSize;
        
        [self callAddCartWebServiceWithProductId:productId andColorId:colorId andSizeId:sizeId andQuantity:quantity andRemark:remark
                                          andColor:color andSize:size];
    }
}
...
/*****************************本地商城商品加入购物车接口**********************************/
- (void)callAddCartWebServiceWithProductId:(long long)productId andColorId:(NSString *)colorId andSizeId:(NSString *)sizeId andQuantity:(int)quantity andRemark:(NSString *)remark andColor:(NSString *)color andSize:(NSString *)size
{
    NSString *ipDomainUrl = [Constant sharedConstant].isRelease?[Constant sharedConstant].domainUrl:[Constant sharedConstant].ipUrl;
    NSString *fuctionName = [NSString stringWithFormat:@"/cart/add"];
    NSString *urlString =  [NSString stringWithFormat:@"%@%@",ipDomainUrl,fuctionName];
    NSLog(@"urlString = %@",urlString);
    NSURL *url = [NSURL URLWithString:urlString];
    ...
    [param setValue:colorId forKey:@"colorId"];
    [param setValue:sizeId forKey:@"sizeId"];
...
/*****************************查询单个商品收藏接口**********************************/
- (void)callSearchFavoriteWebServiceWithProductId:(long long)productId
{
    NSString *ipDomainUrl = [Constant sharedConstant].isRelease?[Constant sharedConstant].domainUrl:[Constant sharedConstant].ipUrl;
    NSString *fuctionName = [NSString stringWithFormat:@"/wishlist/check"];
    NSString *urlString =  [NSString stringWithFormat:@"%@%@",ipDomainUrl,fuctionName];
    NSURL *url = [NSURL URLWithString:urlString];
    NSLog(@"urlString = %@",urlString);
    ...
    [request setTimeOutSeconds:kTimeInterval];
    [request setDelegate:self];
    [request setDidFailSelector:@selector(requestSearchFavoriteFailed:)];
    [request setDidFinishSelector:@selector(requestSearchFavoriteFinished:)];
...
- (void)requestSearchFavoriteFinished:(ASIHTTPRequest *)request
{
    if (![request error])
    {
        NSString *responseString =[request responseString];
...
- (void)requestSearchFavoriteFailed:(ASIHTTPRequest *)request
{
    request.hud = [[MBProgressHUD alloc] initWithView:self.view];
    request.hud.customView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:[NSString stringWithFormat:@"error"]]];
    request.hud.mode = MBProgressHUDModeCustomView;
...
/*****************************点击收藏接口**********************************/
- (void)callFavoriteWebServiceWithProductId:(long long)productId
{
    NSString *ipDomainUrl = [Constant sharedConstant].isRelease?[Constant sharedConstant].domainUrl:[Constant sharedConstant].ipUrl;
    NSString *fuctionName = [NSString stringWithFormat:@"/wishlist/add"];
    NSString *urlString =  [NSString stringWithFormat:@"%@%@",ipDomainUrl,fuctionName];
    NSURL *url = [NSURL URLWithString:urlString];
    NSLog(@"urlString = %@",urlString);
    ...
    [request setDidFailSelector:@selector(requestFavoriteFailed:)];
    
    [request setDidFinishSelector:@selector(requestFavoriteFinished:)];
