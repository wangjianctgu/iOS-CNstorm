...
NSString *const TableViewCellIdentifier = @"GoodsListTableViewCell";
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self)
    {
        ...
        self.categoryName = [NSString stringWithFormat:@"商品分类"];
    }
    return self;
}
...
- (void)viewDidLoad
{
    ...
    //UISearchBar
    self.navigationItem.title = [NSString stringWithFormat:@"%@",self.categoryName];
...
- (void)initMyView
{
    ...
    [self.tableView registerClass:[GoodsListTableViewCell class] forCellReuseIdentifier:TableViewCellIdentifier];
    [self addHeader];
    [self addFooter];
}
...
- (void)addHeader
{
    __unsafe_unretained GoodsListViewController *goodsListViewController = self;
    ...
    header.beginRefreshingBlock = ^(MJRefreshBaseView *refreshView)
    {
        ...
        [goodsListViewController performSelector:@selector(doneWithView:) withObject:refreshView afterDelay:2.0];
...
{
    GoodsListTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:TableViewCellIdentifier forIndexPath:indexPath];
    
    if (cell==nil)
    {
        cell=[[GoodsListTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:TableViewCellIdentifier];
    }
...
- (void)requestFinished:(ASIHTTPRequest *)request
{
    ...
    if (![request error])
    {
        ...
        NSLog(@"good list responseString = %@",responseString);
...
- (BOOL)resolve:(NSArray *)goodsList
{
    ...
    for(NSDictionary *goodsDictionary in goodsList)
    {
        ...
        product.image =  [NSString stringWithString:[[NSString stringWithFormat:@"%@",[goodsDictionary objectForKey:@"image"]] stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];//汉字转换
