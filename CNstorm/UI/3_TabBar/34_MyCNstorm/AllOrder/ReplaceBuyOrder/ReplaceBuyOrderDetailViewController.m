//
//  ReplaceBuyOrderDetailViewController.m
//  CNstorm
//
...
@interface ReplaceBuyOrderDetailViewController ()

@end

@implementation ReplaceBuyOrderDetailViewController

- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil];
    if (self) {
        self.replaceBuyOrder = [[ReplaceBuyOrder alloc]init];
    }
    return self;
}
...
{
    [super viewDidLoad];
    self.navigationItem.title = @"代购订单详情";
...
- (void)initMyView
{
    ...
    float goodsListTableViewHeight = self.replaceBuyOrder.goodsList.count*100.0f + 37.5f;//TableView section+cell的高度
    //商品列表TableView的加载
    self.goodsListTableView = [[UITableView alloc]initWithFrame:CGRectMake(0,0, 320, goodsListTableViewHeight) style:UITableViewStylePlain];
    ...
    float orderInfoViewHeight = 150.0f;
    ...
    receviceAddressInfoLabel.text = self.replaceBuyOrder.receiveAddress;
    ...
    mailCodeInfoLabel.text = self.replaceBuyOrder.mailCode;
    ...
    recevicerInfoLabel.text = self.replaceBuyOrder.receiver;
    ...
    telePhoneInfoLabel.text = self.replaceBuyOrder.telePhone;
    ...
    storeNameInfoLabel.text = [NSString stringWithFormat:@"%@",self.replaceBuyOrder.storeName];
    ...
    UILabel *yunfeiLabel = [[UILabel alloc]initWithFrame:CGRectMake(20.0f, 60.0f, 70.0f, 30.0f)];
    ...
    yunfeiLabel.font = [UIFont systemFontOfSize:13.0f];
    ...
    [orderInfoView addSubview:yunfeiLabel];
    
    UILabel *yunfeiInfoLabel = [[UILabel alloc]initWithFrame:CGRectMake(90.0f, 60.0f, 200.0f, 30.0f)];
    yunfeiInfoLabel.text = [NSString stringWithFormat:@"￥%.2f",self.replaceBuyOrder.yunfei];
    yunfeiInfoLabel.textColor = [UIColor colorWithRed:(180.0f/255.0f) green:(180.0f/255.0f) blue:(180.0f/255.0f) alpha:1.0f];
    yunfeiInfoLabel.font = [UIFont systemFontOfSize:13.0f];
    yunfeiInfoLabel.lineBreakMode = NSLineBreakByTruncatingTail;
    yunfeiInfoLabel.numberOfLines = 1;
    yunfeiInfoLabel.textAlignment = NSTextAlignmentLeft;
    yunfeiInfoLabel.adjustsFontSizeToFitWidth = YES;
    yunfeiInfoLabel.baselineAdjustment = UIBaselineAdjustmentNone;
    [orderInfoView addSubview:yunfeiInfoLabel];

    
    UILabel *orderNoLabel = [[UILabel alloc]initWithFrame:CGRectMake(20.0f, 90.0f, 70.0f, 30.0f)];
    ...
    UILabel *wayNoInfoLabel = [[UILabel alloc]initWithFrame:CGRectMake(90.0f, 90.0f, 200.0f, 30.0f)];
    wayNoInfoLabel.text = self.replaceBuyOrder.orderNo;
    ...
    UILabel *orderDateLabel = [[UILabel alloc]initWithFrame:CGRectMake(20.0f, 120.0f, 70.0f, 30.0f)];
    ...
    UILabel *orderDateInfoLabel = [[UILabel alloc]initWithFrame:CGRectMake(90.0f, 120.0f, 200.0f, 30.0f)];
    orderDateInfoLabel.text = self.replaceBuyOrder.orderDate;
    ...
    UILabel *orderAllCostLabel = [[UILabel alloc]initWithFrame:CGRectMake(10.0f, 10.0f, 110.0f, 30.0f)];
    orderAllCostLabel.text = [NSString stringWithFormat:@"总计:￥%.2f",self.replaceBuyOrder.orderAllCost];
    orderAllCostLabel.textColor = [UIColor colorWithRed:(251.0f/255.0f) green:(110.0f/255.0f) blue:(83.0f/255.0f) alpha:1.0f];
    orderAllCostLabel.font = [UIFont systemFontOfSize:14.0f];
    orderAllCostLabel.lineBreakMode = NSLineBreakByTruncatingTail;
    orderAllCostLabel.numberOfLines = 1;
    orderAllCostLabel.textAlignment = NSTextAlignmentLeft;
    orderAllCostLabel.adjustsFontSizeToFitWidth = YES;
    orderAllCostLabel.baselineAdjustment = UIBaselineAdjustmentNone;
    [orderStatusView addSubview:orderAllCostLabel];
    ...
    UIButton *orderStatusButton = [[UIButton alloc]initWithFrame:CGRectMake(140.0f, 10.0f, 75.0f,30.0f)];
    ...
    if (self.replaceBuyOrder.orderStatus == 1)
    {
        [orderStatusButton addTarget:self action:@selector(orderPayView:) forControlEvents:UIControlEventTouchUpInside];
    ...
    if (self.replaceBuyOrder.expressNumber.length != 0 || ![self.replaceBuyOrder.expressNumber isEqualToString:@""])
    {
    ...
    {
        [orderStatusButton setFrame:CGRectMake(230.0f, 10.0f, 75.0f, 30.0f)];
    }
}
...
- (void)orderPayView:(id)sender
{
    ...
    orderPayViewController.orderIdsStr = self.replaceBuyOrder.orderNo;
    orderPayViewController.orderTotalCost = self.replaceBuyOrder.orderAllCost;
    [self.navigationController pushViewController:orderPayViewController animated:YES];
}

- (void)expressShowView:(id)sender
{
    NSString *url = [NSString stringWithFormat:@"http://m.kuaidi100.com/index_all.html?type=%@&postid=%@",self.replaceBuyOrder.express,self.replaceBuyOrder.expressNumber];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    if (self.replaceBuyOrder.goodsList.count == 0)
    {
    ...
    return self.replaceBuyOrder.goodsList.count;//每个订单，只显示1件商品
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    headerView.headerLabel.text = [NSString stringWithFormat:@"%@",self.replaceBuyOrder.storeName];
    return headerView;
}
...
{
    static NSString *ReplaceBuyOrderDetailTableViewCellIdentifier = @"ReplaceBuyOrderDetailTableViewCellIdentifier";
    
    ReplaceBuyOrderDetailTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ReplaceBuyOrderDetailTableViewCellIdentifier];
    
    if (cell==nil)
    {
        cell=[[ReplaceBuyOrderDetailTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ReplaceBuyOrderDetailTableViewCellIdentifier];
    }
    
    [cell setSelectionStyle:UITableViewCellSelectionStyleNone];
    
    
    cell.goods = self.replaceBuyOrder.goodsList[indexPath.row];
