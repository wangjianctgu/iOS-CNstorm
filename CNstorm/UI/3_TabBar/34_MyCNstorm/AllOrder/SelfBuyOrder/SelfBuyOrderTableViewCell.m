...
@implementation SelfBuyOrderTableViewCell
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self)
    {
        ...
        [self.orderStatusButton setTitle:@"订单状态" forState:UIControlStateNormal];
        ...
        UIView *footerView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, 98.0f, 320.0f, 12.0f)];//创建一个视图
...
- (void)layoutSubviews
{
    ...
    [self.orderImageView setImageURLStr:self.selfBuyOrder.orderImage  placeholder:[UIImage imageNamed:@"default75.png"]];
    self.orderImageView.clipsToBounds = YES;
    self.orderImageView.contentMode = UIViewContentModeScaleAspectFill;
    
    self.orderTitleLabel.text = self.selfBuyOrder.orderTitle;
    self.orderAllCostLabel.text = [NSString stringWithFormat:@"￥%.2f",self.selfBuyOrder.orderAllCost];
    self.orderGoodTypeCountLabel.text = [NSString stringWithFormat:@"共%d件商品",self.selfBuyOrder.orderGoodTypeCount];
