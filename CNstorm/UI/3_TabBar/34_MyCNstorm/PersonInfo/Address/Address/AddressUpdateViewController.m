...
- (void)initMyView
{
    ...
    self.recevicerTextField.text = self.address.recevicer;
    ...
    self.telePhoneTextField.text = self.address.telePhone;
    ...
    self.mailCodeTextField.text = self.address.mailCode;
    ...
    self.mailCodeTextField.autocorrectionType = UITextAutocorrectionTypeNo;//拼写检查
    ...
    self.countryLabel.tag = self.address.countryId;
    ...
    self.provinceLabel.text = self.address.province;
    self.provinceLabel.tag = self.address.provinceId;
    ...
    self.addressDetailTextView.text = self.address.addressDetail;
    self.addressDetailTextView.textColor = [UIColor colorWithRed:(153.0f/255.0f) green:(153.0f/255.0f) blue:(153.0f/255.0f) alpha:1.0f];
    ...
    self.addressDetailTextView.keyboardType = UIKeyboardTypeDefault;
...
- (void)callUpdateWebService
{
    ...
    [param setValue:[NSString stringWithFormat:@"%d", self.address.addressId]forKey:@"addressId"];
...
- (void)requestDeleteOrderFinished:(ASIHTTPRequest *)request
{
    ...
    if (![request error])
    {
        ...
        if(resultCode == 1)
        {
            ...
            request.hud.labelText = @"保存成功";
            
            [self popToRootView];
            
            if (self.delegate && [self.delegate respondsToSelector:@selector(didFinishedReturn:andHud:)])
            {
                [self.delegate didFinishedReturn:self andHud:request.hud];
            }
        }
        else
        {
