...
- (void)initMyView
{
    ...
    commitLabel.text = @"订单已经成功提交，请尽快完成支付！";
    ...
    orderNoLabel.text = [NSString stringWithFormat:@"订单编号:%@",self.orderIdsStr];
    ...
    self.moneyLabel.text = [NSString stringWithFormat:@"应付金额:%.2f",self.orderTotalCost];
...
- (void)commit:(id)sender
{
    ...
    {
        if(self.yuer < self.orderTotalCost)
        {
    ...
    if (self.thirdPayButton.selected)
    {
        ...
        thirdPayViewController.totalCost = self.orderTotalCost;
        thirdPayViewController.idsStr = self.orderIdsStr;
        thirdPayViewController.payType = 0;
...
- (void)yuerPayWebService
{
    ...
    Customer *customer = [NSKeyedUnarchiver unarchiveObjectWithData:[[NSUserDefaults standardUserDefaults] objectForKey:@"customer"]];
    NSMutableDictionary *param = [[NSMutableDictionary alloc]init];
    [param setValue:[NSString stringWithFormat:@"%lld",customer.customerid] forKey:@"customerId"];
    [param setValue:self.orderIdsStr forKey:@"orderIds"];
    [param setValue:[NSString stringWithFormat:@"%.2f",self.orderTotalCost] forKey:@"money"];
...
- (void)requestYuerPayFinished:(ASIHTTPRequest *)request
{
    ...
    if (![request error])
    {
        ...
        else
        {
            ...
            payComViewController.payType = 0;
