...
NSString *const AwaitReceiveTableViewCellIdentifier = @"AwaitReceiveTableViewCellIdentifier";

@interface AwaitReceiveViewController ()
{
...
@implementation AwaitReceiveViewController
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title = @"待收货国际运单";
    ...
    [self initAwaitReceiveWayTableView];
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    badgeKeyValue.tabBadge4 = badgeKeyValue.tabBadge4 - badgeKeyValue.awaitReceiveBadge;
    badgeKeyValue.awaitReceiveBadge = 0;
...
//-------------------------------国际运单TableView初始化------------------------//
- (void)initAwaitReceiveWayTableView
{
    //国际运单TableView的加载
    self.wayTableView = [[UITableView alloc]initWithFrame:CGRectMake(0.0f, 0.0f,MainScreenWidth, MainScreenHeight-NavigationBarHeight) style:UITableViewStylePlain];
    self.wayTableView.delegate = self;
    self.wayTableView.dataSource = self;
    [self.view addSubview:self.wayTableView];
    
    // 1.注册
    [self.wayTableView registerClass:[AllWayTableViewCell class] forCellReuseIdentifier:AwaitReceiveTableViewCellIdentifier];
...
- (void)addHeader
{
    __unsafe_unretained AwaitReceiveViewController *awaitReceiveViewController = self;
    ...
    header.beginRefreshingBlock = ^(MJRefreshBaseView *refreshView)
    {
        ...
        [awaitReceiveViewController performSelector:@selector(doneWithView:) withObject:refreshView afterDelay:2.0f];
...
- (void)addFooter
{
    ...
    footer.beginRefreshingBlock = ^(MJRefreshBaseView *refreshView)
    {
        ...
        {
            self.value = (int)(self.wayList.count/10)+1;
            [self callAllWayListWebService];
        }
...
{
    return nil;//国际运单wayTableViewCell 没有headView
}
...
{
    AllWayTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:AwaitReceiveTableViewCellIdentifier forIndexPath:indexPath];
    
    if (cell == nil)
    {
        cell = [[AllWayTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:AwaitReceiveTableViewCellIdentifier];
    }
...
{
    UIAlertView *wayAlertView = [[UIAlertView alloc]initWithTitle:@"确认删除该订单吗？" message:@"" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
...
- (void)callAllWayListWebService
{
    ...
    [param setValue:[NSString stringWithFormat:@"3"] forKey:@"status"];
...
- (void)requestDeleteWayFinished:(ASIHTTPRequest *)request
{
    ...
    if (![request error])
    {
        ...
        NSLog(@"DeleteWay responseString = %@",responseString);
